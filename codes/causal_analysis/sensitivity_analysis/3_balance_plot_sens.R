...
############################# 1. data manipulation ############################
# load data
birth <- fread(paste0(dir_input, "birth_final.csv"), drop = "V1")
...
ipw <- fread(paste0(dir_input, "birth_ipwsens.csv"),
             select = c("V1","bc_30d.wt.t", "bc_3090d.wt.t", "bc_90280d.wt.t"))

# merge IPW into birth data
birth_all <- left_join(birth, ipw, by=c("uniqueid_yr" = "V1"))
fwrite(birth_all, file = paste0(dir_output, "birth_allsens.csv"))
rm(birth)
rm(ipw)
gc()
...
############################# 2. check balance ###############################
## load data
birth_all <- fread(paste0(dir_input, "birth_allsens.csv"))
...
############################# 2.1 bc_30d #####################################
x <- balanceALL %>% select(-bc_30d, -bc_30d.wt.t, -bc_3090d.wt.t, -bc_90280d.wt.t)
name.x <- names(x)
x <- as.data.frame(x)
...
head(balance_30d)
write.csv(balance_30d, file = "balance_30dsens.csv")

############################# 2.2 bc_3090d #####################################
x <- balanceALL %>% select(-bc_3090d, -bc_30d.wt.t, -bc_3090d.wt.t, -bc_90280d.wt.t)
name.x <- names(x)
x <- as.data.frame(x)
unweightedCor <- matrix(NA,dim(x)[2],1)
weightedCor <- matrix(NA,dim(x)[2],1)
...
head(balance_3090d)
write.csv(balance_3090d, file = "balance_3090dsens.csv")
...
head(balance_90280d)
write.csv(balance_90280d, file = "balance_90280dsens.csv")
...
balance_30d <- read.csv(file = paste0(dir_output, "balance_30dsens.csv"),
                        row.names = 1)
balance_3090d <- read.csv(file = paste0(dir_output, "balance_3090dsens.csv"),
                        row.names = 1)
balance_90280d <- read.csv(file = paste0(dir_output, "balance_90280dsens.csv"),
                        row.names = 1)

pdf(file = paste0(dir_output,"balance_30dsens.pdf"))
...
pdf(file = paste0(dir_output,"balance_3090dsens.pdf"))
...
pdf(file = paste0(dir_output,"balance_90280dsens.pdf"))
...
pdf(file = paste0(dir_output,"balance_3090dsens_display.pdf"))
