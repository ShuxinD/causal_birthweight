...
# Output: birth dataset with merged SES variables                             #
# Author: Shuxin Dong                                                         #
# Date: Oct 21, 2020                                                          #
###############################################################################
...
## load data
birth_13 <- fread(paste0(dir_input_birth, "birth_w_geocode13.csv"),
                  drop = c("V1"),
                  colClasses = c(rep(NA, 5), rep("character",1)),
                  na.strings = "NA")
birth_14 <- fread(paste0(dir_input_birth, "birth_w_geocode14.csv"),
                  drop = c("V1"),
                  colClasses = c(rep(NA, 5), rep("character",1)),
                  na.strings = "NA")
birth_15 <- fread(paste0(dir_input_birth, "birth_w_geocode15.csv"),
                  drop = c("V1"),
                  colClasses = c(rep(NA, 5), rep("character",1)),
                  na.strings = "NA")
...
# write.csv(birth_0010missing, file = paste0(dir_input_birth, "birth_0010missing.csv"))
...
mhvalue13$mhvalue <- as.numeric(mhvalue13$mhvalue)
...
birth_13 <- left_join(birth_13, mhincome13, by="geoid2013")
birth_13 <- left_join(birth_13, mhvalue13, by="geoid2013")
birth_13 <- left_join(birth_13, poverty13, by="geoid2013")
birth13_SES <- birth_13[,.(uniqueid_yr, year, mhincome, mhvalue, percentPoverty)]
write.csv(birth13_SES, file = paste0(dir_output_birth, "birth13_SES.csv"))
...
mhvalue14$mhvalue <- as.numeric(mhvalue14$mhvalue)
...
poverty14$percentPoverty <- as.numeric(poverty14$percentPoverty)
poverty14 <- na.omit(poverty14)
poverty14[, geoid2014 := substr(GEO_ID,10,20)]
poverty14

birth_14 <- left_join(birth_14, mhincome14, by="geoid2014")
birth_14 <- left_join(birth_14, mhvalue14, by="geoid2014")
birth_14 <- left_join(birth_14, poverty14, by="geoid2014")
birth14_SES <- birth_14[,.(uniqueid_yr, year, mhincome, mhvalue, percentPoverty)]
write.csv(birth14_SES, file = paste0(dir_output_birth, "birth14_SES.csv"))
...
mhincome15$mhincome <- as.numeric(mhincome15$mhincome)
...
write.csv(birth15_SES, file = paste0(dir_output_birth, "birth15_SES.csv"))
...
birth_12 <- left_join(birth_12, mhincome12, by="geoid2010")
birth_12 <- left_join(birth_12, mhvalue12, by="geoid2010")
birth_12 <- left_join(birth_12, poverty12, by="geoid2010")
birth12_SES <- birth_12[,.(uniqueid_yr, year, mhincome, mhvalue, percentPoverty)]
write.csv(birth12_SES, file = paste0(dir_output_birth, "birth12_SES.csv"))
...
birth_11 <- left_join(birth_11, mhincome11, by="geoid2010")
birth_11 <- left_join(birth_11, mhvalue11, by="geoid2010")
birth_11 <- left_join(birth_11, poverty11, by="geoid2010")
birth11_SES <- birth_11[,.(uniqueid_yr, year, mhincome, mhvalue, percentPoverty)]
summary(birth11_SES)
write.csv(birth11_SES, file = paste0(dir_output_birth, "birth11_SES.csv"))
